...
The following will be a specific presentation of graph theory in the context of urban theory. Graphs have long been used to represent urban networks, and the following will explain some of that history, as well as the unique way in which this project uses urban data to construct graphs. The graph of constructs which are or contain said data will be referred to as the "projection" of the constructs.
...
Projecting a construct and its edges requires we first know something about the type of analysis we want to perform, as the spectral analysis step is highly dependent on choosing the correct edge arrangement that reflects the spatial context. Critically, edges must represent dependency between constructs, and their weight must reflect that dependency. While realistically all constructs are dependent on all other constructs in the built environment, constraints must be placed on edge drawing conditions and weighting to have the graph conform to best practices in urban theory. The conditions that bound edge drawing will be refered to as the edge kernel, which is one of the primary parameters we will use in analysis. At its most simple, the edge kernel could be a distance (e.g. measured parcel centroid-to-centroid), a minimum interaction (e.g. traffic), but theoretically does not need to include the whole set of vertexes.
